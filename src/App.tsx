...
import {
  Typography, TooltipProvider, Badge,
...
import { DemandModelContext, useDemandModelProvider } from './hooks/useDemandModel';
...
function App() {
  ...
  const demandModelState = useDemandModelProvider();
  const training = useRlTraining();
  const gbrtTraining = useGbrtTraining();
  ...
  useEffect(() => {
    if (gbrtTraining.isComplete && gbrtTraining.model) {
      demandModelState.setModel(gbrtTraining.model);
    }
  }, [gbrtTraining.isComplete, gbrtTraining.model]); // eslint-disable-line react-hooks/exhaustive-deps

  // Auto-completion: RL trained → mark 'training' complete, unlock results + explainability
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    gbrtTraining.reset();
  }, [csvData.datasetName]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const handleLearnNavigate = useCallback((step: string) => {
    ...
    if (pipeline.canNavigateTo(pipelineStep as PipelineStep)) {
      pipeline.setActiveStep(pipelineStep as PipelineStep);
    }
    setLearnOpen(false);
  }, [pipeline]);
  ...
  const renderActiveStep = () => {
    switch (pipeline.activeStep) {
      ...
      case 'data':
        return <DataExplorer />;
      case 'demand-model':
        return <DemandModel gbrtTraining={gbrtTraining} onComplete={handleDemandModelComplete} />;
      case 'training':
  ...
  return (
    <TooltipProvider>
      <CsvDataContext.Provider value={csvData}>
        <DemandModelContext.Provider value={demandModelState}>
            ...
            >
                  ...
                  <div className="flex items-center" style={{ gap: '8px' }}>
                    ...
                    <button
                      ...
                      style={{
                        ...
                        borderRadius: '6px',
                        border: '1px solid var(--color-subtle)',
                        backgroundColor: learnOpen ? 'var(--color-interactive)' : 'var(--color-base-white)',
                        color: learnOpen ? 'white' : 'var(--color-dark)',
                        fontSize: '13px',
                        fontWeight: 500,
                        cursor: 'pointer',
                        transition: 'all 0.15s',
              ...
              >
                <PipelineStepper
                  ...
                  canNavigateTo={pipeline.canNavigateTo}
                  onStepClick={pipeline.setActiveStep}
                />
              </div>
